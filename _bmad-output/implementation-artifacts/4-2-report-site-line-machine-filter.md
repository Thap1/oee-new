...
- [ ] Task 1: Backend — extend `OeeReportQueryUseCase.GetReportAsync` with an optional Site/Line/Machine filter (AC: #1, #2, #3)
  ...
  - [ ] Extend `OeeReportQueryUseCase.GetReportAsync` (`src/OeeNew.Application/Reports/OeeReportQueryUseCase.cs`, Story 4.1) with two new optional parameters: `ReportFilterTargetType? filterType, Guid? filterId`. Resolution order, composed from what Story 4.1 already built:
...
- [ ] Task 4: Testing (all AC)
  ...
  - [ ] `web/oee-shell/src/app/pages/reports/reports-page.spec.ts` (extend, Story 4.1): selecting a filter re-fetches with the right query params; filter dropdown options come from `ScopeService`'s already-scoped `sites()`/`lines()` signals, not a fresh unscoped fetch.
